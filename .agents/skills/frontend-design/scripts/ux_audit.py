...
class UXAuditor:
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'ui', '.venv', 'arbor', 'arbor_example_benchmark', '.agents'}]
            if os.path.abspath(root) == abs_dir:
                # Skip auditing files directly in the root workspace directory
                continue
            for file in files:
